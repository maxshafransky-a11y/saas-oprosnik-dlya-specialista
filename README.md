...
При выборе учебного кейса мне сразу пришла мысль о собственном проекте: мне нужен первичный опросник для платформы, где специалист ведёт клиентов по вопросам физического и ментального здоровья.

Поэтому я немного изменил направление задачи. Вместо универсального сервиса-конструктора опросов сделал прикладной SaaS-модуль под свои реальные потребности и дальнейшие разработки. Этот опросник должен стать первым шагом клиентского пути: собрать исходные данные, сохранить их в защищённой карточке и передать их специалисту для дальнейшей работы — вручную и с помощью AI-агентов.

Такой вариант кейса выбран потому, что он решает конкретную проблему большого проекта: клиенту не нужно долго регистрироваться или разбираться в сложном интерфейсе, а специалист получает структурированные ответы и документы в едином формате.
...
Скриншоты можно положить в `docs/screenshots/`. Для скринкаста и отчёта рекомендуется снять следующие кадры:

| Файл | Что должно быть видно |
|---|---|
| `01-project-structure.png` | Структура репозитория и основные каталоги |
| `02-consent.png` | Стартовая ссылка и отдельная галочка согласия |
| `03-email-access.png` | Ввод email и экран отправки письма |
| `04-mailpit.png` | Письмо с magic link и OTP-кодом в Mailpit |
| `05-questionnaire-start.png` | Первый раздел, прогресс и широкие карточки вопросов |
| `06-questionnaire-choice.png` | Single-choice: выбран только один вариант и доступен комментарий |
| `07-questionnaire-mobile.png` | Адаптивный вид на мобильной ширине |
| `08-file-upload.png` | Загрузка PDF и статус проверки файла |
| `09-review.png` | Страница проверки всех ответов перед отправкой |
| `10-complete.png` | Подтверждение отправки анкеты |
| `11-editing.png` | Возврат к редактированию после отправки |
| `12-test-result.png` | Успешный результат Playwright/pytest |

На каждом кадре лучше оставлять адрес страницы или часть интерфейса, по которой проверяющий понимает текущий шаг.
